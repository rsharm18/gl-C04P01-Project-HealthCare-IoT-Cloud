    ...
    def aggregate_device_data_for_device(self, deviceid, items: List):
        ...
        while start_time < self._to_date:
            ...
            for device_type in sensors:
                ...
                # print(" device_type :: ", device_type, " len(filtered_data_set) ", len(filtered_data_set))
                if len(filtered_data_set) > 0:
                    ...
                    # inser the aggregated data into dynamodb tbl
                    agdm = Aggregated_Data_Model(deviceid, device_type, format_decimal_digits(avg),
                                                 format_decimal_digits(minm), format_decimal_digits(maxm), start_time,
                                                 end_time)
        ...
        # print("Data Count for all sensors ", len(device_data), " aggregated data size ", len(aggregated_data))  # print("End aggregating data for :: ", deviceid)  # print("deviceId ", deviceid)  # print("device_data ", device_data)  # print("aggregated data ", aggregated_data)
